  ...
  return (
    <div>
      <h2 id="attachment-heading" data-cy="AttachmentHeading">
        <Translate contentKey="odiseyDocApp.attachment.home.title">Attachments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="odiseyDocApp.attachment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/attachment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="odiseyDocApp.attachment.home.createLabel">Create new Attachment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {attachmentList && attachmentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="odiseyDocApp.attachment.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('description')}>
                  <Translate contentKey="odiseyDocApp.attachment.description">Description</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('file')}>
                  <Translate contentKey="odiseyDocApp.attachment.file">File</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="odiseyDocApp.attachment.dodument">Dodument</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {attachmentList.map((attachment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{attachment.dodument ? <Link to={`/dodument/${attachment.dodument.id}`}>{attachment.dodument.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="odiseyDocApp.attachment.home.notFound">No Attachments found</Translate>
            </div>
          )
        )}
      </div>
